...
1) script 태그로 로딩된 model.js에 선언된 변수는 전역변수로 취급되어 등록된다. 여기서 ProductModel도 전역적으로 사용할 수 있는 클래스(=변수) (맞나?!)가 된다. 
2) script 태그 로 선언된 파일이 '동기' 방식으로 로딩되어 페이지의 렌더링 속도를 저하하는 원인이 된다. 
